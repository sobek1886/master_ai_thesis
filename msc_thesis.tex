...
% natbib's \bibsection sets the running head via \@mkboth, which does not take
% here, so the bibliography pages inherit the Conclusion chapter's mark. Force
% the mark directly.
...
% Draft annotations. \showdraftnotestrue renders them in the margin colour
% for working on the document; \showdraftnotesfalse (the default) hides
% them so the compiled PDF is submission-ready. The notes stay in the
% source either way.
...
\period{05/01/2026 - 12/08/2026}
\credits{36 ECTS}
\examiner{Dr Marieke Schouwstra}
\dailysupervisor{Casper Thuis}
\uvasupervisor{Dr James Trujillo}
\addsupervisor{Angelos Nalmpantis}
...
motion-locked avatar re-renders of NGT744, a self-recorded Dutch Sign
Language (NGT) corpus, and generative per-frame appearance edits of ASL
Citizen videos. The outcome splits into two regimes.
In the low-diversity NGT744 setting (4 training signers, one studio
...
an informative null: with hundreds of pretraining signers and 35
training signers, appearance invariance is already bought by real
diversity.
