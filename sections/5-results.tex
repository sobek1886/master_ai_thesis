...
intervention on NGT where synthetic appearance augmentation produces a
clear, sizeable gain (\S\ref{sec:results-crossdomain}); third, a high-diversity setting
intervention on ASL Citizen where the same idea does not lead to improvements(\S\ref{sec:results-indomain}).
...
\hline
\end{tabular}
\caption{SignCLIP counterfactual analysis on WLASL: mean cosine similarity
by pair type.}
...
\hline
\end{tabular}
\caption{Cross-gloss signer retrieval probe, SignCLIP embeddings vs.\ raw
...
\caption{Signer-separability probes on raw ASL Citizen features (train
...
\section{Cross-domain intervention: NGT}
\label{sec:results-crossdomain}

\subsection{Isolating the appearance axis: single-view arms}
\label{sec:ngt-single-view}
Synthetic multi-view training bundles two axes at once: added
\emph{identities} (avatars or edits) and added \emph{viewpoints} (cameras).
The single-view ladder holds viewpoint fixed and varies appearance
only. Its baseline trains on the front camera alone ($K{=}1$; with no
positive pairs, SupCon degenerates, so this arm uses the video-only
InfoNCE of \S\ref{sec:method-objectives});
the treatment arms add, at the \emph{same} front camera, either both Unreal
avatars ($K{=}3$) or two Flux identity edits (signer swap, skin tone;
$K{=}3$) --- a head-to-head of the parametric and diffusion routes at
matched $K$, matched content, and matched viewpoint --- plus a
full-strength arm with all six avatar views ($K{=}7$). All arms share an
identical sentence set --- the 744 sentences of the full four-signer pool
with complete synthetic coverage
(Table~\ref{tab:ngt_setups}) --- sample one repeat take per step, and keep the
contrastive batch constant (64 signs, so the number of in-batch negatives is
equal across arms; the encoder-pass cost instead scales with $K$), with the
learning-rate schedule matched to the per-arm update count.
Evaluation is fixed across arms: front-view, take-0 galleries and the
held-out real test set (the frozen 364 sentences of the current test pool
that have a trained counterpart; train and test counts are no longer
matched (the frozen 364-sentence protocol of Table~\ref{tab:ngt_setups}), scored in both directions with
per-sentence ranks, enabling paired comparisons (exact McNemar on top-1 correctness plus
a bootstrap CI on the R@1 difference) rather than seed-mean gaps alone.
\begin{table*}[t]
\centering
\small
\begin{tabular}{lcccc}
\toprule
\textbf{Arm} & \textbf{R@1} & \textbf{R@5} & \textbf{R@10} & \textbf{MRR} \\
\midrule
\multicolumn{5}{l}{\textit{Test $\to$ Train \; (query: test set, gallery: train set)}} \\
\midrule
E1 real ($K{=}1$)            & 0.234\pmsd{0.025} & 0.420\pmsd{0.040} & 0.506\pmsd{0.054} & 0.325\pmsd{0.027} \\
E2 real multiview ($K{=}3$)  & 0.266\pmsd{0.013} & 0.453\pmsd{0.024} & 0.544\pmsd{0.021} & 0.361\pmsd{0.002} \\
\rowcolor{rowhl}
E3 $+$Unreal ($K{=}3$)       & \textbf{0.649}\pmsd{0.044} & 0.836\pmsd{0.026} & 0.899\pmsd{0.018} & 0.738\pmsd{0.033} \\
E4 $+$Flux ($K{=}3$)         & 0.505\pmsd{0.010} & 0.729\pmsd{0.010} & 0.808\pmsd{0.011} & 0.607\pmsd{0.009} \\
E5 $+$Unreal full ($K{=}7$)  & 0.437\pmsd{0.018} & 0.676\pmsd{0.019} & 0.783\pmsd{0.032} & 0.553\pmsd{0.018} \\
E6 $+$Flux full ($K{=}4$)    & 0.491\pmsd{0.025} & 0.713\pmsd{0.028} & 0.788\pmsd{0.030} & 0.593\pmsd{0.020} \\
E7 $+$Flux glasses ($K{=}5$) & 0.461\pmsd{0.023} & 0.691\pmsd{0.015} & 0.771\pmsd{0.023} & 0.568\pmsd{0.012} \\
E8 combined ($K{=}10$)       & 0.525\pmsd{0.022} & 0.770\pmsd{0.016} & 0.855\pmsd{0.018} & 0.636\pmsd{0.021} \\
E9 Unreal-only ($K{=}3$)     & 0.296\pmsd{0.022} & 0.545\pmsd{0.014} & 0.662\pmsd{0.027} & 0.414\pmsd{0.018} \\
E10 Flux-only ($K{=}3$)      & 0.452\pmsd{0.044} & 0.671\pmsd{0.050} & 0.750\pmsd{0.039} & 0.554\pmsd{0.039} \\

\midrule
\multicolumn{5}{l}{\textit{Train $\to$ Test \; (query: train set, gallery: test set)}} \\
\midrule
E1 real ($K{=}1$)            & 0.247\pmsd{0.035} & 0.446\pmsd{0.043} & 0.547\pmsd{0.037} & 0.347\pmsd{0.037} \\
E2 real multiview ($K{=}3$)  & 0.420\pmsd{0.016} & 0.721\pmsd{0.018} & 0.805\pmsd{0.005} & 0.551\pmsd{0.014} \\
\rowcolor{rowhl}
E3 $+$Unreal ($K{=}3$)       & \textbf{0.695}\pmsd{0.030} & 0.886\pmsd{0.025} & 0.934\pmsd{0.015} & 0.779\pmsd{0.024} \\
E4 $+$Flux ($K{=}3$)         & 0.606\pmsd{0.006} & 0.827\pmsd{0.025} & 0.889\pmsd{0.011} & 0.705\pmsd{0.002} \\
E5 $+$Unreal full ($K{=}7$)  & 0.482\pmsd{0.025} & 0.766\pmsd{0.022} & 0.859\pmsd{0.030} & 0.615\pmsd{0.023} \\
E6 $+$Flux full ($K{=}4$)    & 0.610\pmsd{0.017} & 0.836\pmsd{0.014} & 0.902\pmsd{0.020} & 0.713\pmsd{0.012} \\
E7 $+$Flux glasses ($K{=}5$) & 0.581\pmsd{0.019} & 0.807\pmsd{0.030} & 0.881\pmsd{0.003} & 0.683\pmsd{0.014} \\
E8 combined ($K{=}10$)       & 0.579\pmsd{0.004} & 0.831\pmsd{0.010} & 0.901\pmsd{0.005} & 0.690\pmsd{0.005} \\
E9 Unreal-only ($K{=}3$)     & 0.252\pmsd{0.017} & 0.484\pmsd{0.059} & 0.596\pmsd{0.063} & 0.367\pmsd{0.032} \\
E10 Flux-only ($K{=}3$)      & 0.513\pmsd{0.034} & 0.755\pmsd{0.012} & 0.837\pmsd{0.020} & 0.625\pmsd{0.020} \\
\bottomrule
\end{tabular}
\caption{Single-view NGT ladder: cross-signer retrieval (0--1 scale) with viewpoint
held to the front camera, mean$\pm$sd over 3 seeds. Arm composition ($K$ =
views per training item; every arm except E9/E10 includes the real
front-view clip): E1 $=$ real front view only (video-only InfoNCE; all
other arms SupCon over the $K$ views); E2 $=$ E1 $+$ 2 further real
camera views; E3 $=$ E1 $+$ 2 Unreal avatars at the front camera; E4 $=$
E1 $+$ 2 Flux identity edits (signer swap, skin tone); E5 $=$ E3 $+$ the
same two avatars at the two remaining cameras; E6 $=$ E4 $+$ the shirt
edit; E7 $=$ E6 $+$ the glasses edit (all four Flux edits); E8 $=$ E5
$+$ E6's three Flux edits; E9 $=$ one Unreal avatar $\times$ 3 cameras,
no real video; E10 $=$ E6's three Flux edits only, no real
video. All arms share one
sentence set (744 sentences, four signers; the 364 with a test counterpart
are evaluated), one repeat take sampled per step, and a constant contrastive
batch (64 signs, i.e.\ equal negatives across arms; the encoder-pass cost
scales with $K$). The learning-rate schedule is matched to the per-tier
update count. Evaluation is fixed across arms (front-view take-0 galleries,
held-out test queries). The length-only shortcut floor is the gallery ranked
by $|\Delta$ clip count$|$.}
...
% Axis scales: both ngt_ladder_r1_vs_k.png and ngt_dropoff.png now render
...
\label{tab:ngt_ladder}
\end{table*}

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{ngt_ladder_r1_vs_k.png}
\caption{Single-view NGT ladder: cross-signer R@1 against the number of
views per training item $K$, by augmentation family (3-seed mean$\pm$sd;
front-view evaluation as in Table~\ref{tab:ngt_ladder}, same 0--1 scale).
Lines trace the
...
edits are added.}
\label{fig:ngt_ladder_r1}
\end{figure}

Figure~\ref{fig:ngt_ladder_r1} plots the ladder's R@1 against $K$ by
augmentation family, which makes the family structure that
Table~\ref{tab:ngt_ladder} lists row-wise directly visible.
With viewpoint held fixed, both synthetic routes recover a large share of the \todo{move descriptions of $EX = E1 + xxx$ to the table}
cross-signer gap. Over the real-only baseline (E1, test$\to$train R@1 $0.234$), the
matched-$K{=}3$ Unreal arm (E3) reaches $0.649$ and the matched-$K{=}3$ Flux arm
(E4) $0.505$; each route's paired gain over E1 is significant on every
seed (exact McNemar $p<0.05$, $3/3$ seeds in both directions: Unreal
E1$\to$E5 $\Delta$R@1 $+0.203$ test$\to$train / $+0.234$ train$\to$test, Flux E1$\to$E6
$+0.257$ / $+0.363$).
\todome{NUMBERS BUG (03.08, verified against tab:ngt\_ladder): the
sentence's subject is the matched-$K{=}3$ arms E3 and E4, but the quoted
deltas are the full-strength comparisons E1$\to$E5 and E1$\to$E6 ---
E3$-$E1 is $0.649-0.234=0.415$ test$\to$train, not $+0.203$. Either pull
the actual E1$\to$E3 and E1$\to$E4 McNemar results from the ladder eval
artifacts (do NOT reuse the E5/E6 values), or reword the sentence to
attribute the quoted significance to E5/E6 and drop the per-route claim
for E3/E4. A reviewer redoing the arithmetic will catch this.}
At matched strength the parametric route wins the head-to-head:
E3 exceeds E4 by $\Delta$R@1 $0.144$ (test$\to$train) / $0.089$ (train$\to$test), significant on
all three seeds test$\to$train and two of three train$\to$test. The diffusion route therefore
retains roughly $65$--$80\%$ (by
direction) of the Unreal appearance gain at equal $K$, content, and viewpoint
--- a substantial but incomplete substitute for the parametric renders. Both
...
identity diversity is. The length-only shortcut floor (R@1 $0.005$) sits two
orders of magnitude below every model arm, so none of these effects is a
duration artefact.
...
Taken together, the ladder's view-composition arms separate what viewpoint
diversity contributes from what identity diversity contributes. Adding real
viewpoints does help: the two extra camera views in E2 lift the real-only
...
viewpoint-matched multiview panel (Table~\ref{tab:ngt_signer_count},
...
to $+0.160$ in the single-view E1$\to$E5 panel) --- real viewpoints already supply part of what the
avatars supply. The viewpoint story is therefore graded, not null: real
...
% Matched-K panel provenance: the E3-equivalent arms (ngt_sv_unreal_R1..R4)
...
add. The NGT corpus permits a direct test --- its training pool comprises
...
$\Delta(S)$ of two Unreal arms over the real-only arm (E1, $K{=}1$): the
matched-$K$ arm (the ladder's E3 setup, $K{=}3$: front camera plus both
avatars' front views) and the full Unreal arm (E5, $K{=}7$).
...
budget the smallest single-signer pool supports --- session 251126 has 182
render-complete sentences), split evenly across its included signers, with a
...
take (the two avatar front views for E3, all six avatar views for E5). Evaluation is the same frozen 364-sentence protocol as the
ladder (Table~\ref{tab:ngt_setups}),
with galleries built from features independent of what was trained on, so
the eval set never varies across conditions --- it therefore contains
sentences outside a given condition's 180-sentence training pool and, for
$S{<}4$, gallery videos of signers absent from that condition's training.


Table~\ref{tab:ngt_signer_count} shows both outcomes at once, split by arm.
The real-only baseline moves little with $S$: adding real signers at a
fixed 180-sentence budget does not by itself improve cross-signer retrieval
(R@1 $0.21$--$0.24$ test$\to$train throughout). The full-Unreal gain
behaves as the two-regime account predicts: $\Delta(S)$ shrinks from
$+0.145$ at $S{=}1$ to $+0.081$ at $S{=}4$ (test$\to$train; $+0.160$ to
$+0.134$ train$\to$test), and the viewpoint-matched multiview panel repeats
the pattern from a stronger real baseline, with smaller gains throughout.
The matched-$K$ arm does not: it is the strongest configuration at every
$S$ (R@1 $0.450$--$0.527$ test$\to$train, $0.523$--$0.557$
train$\to$test), its gain is two to three times the full-Unreal gain
($+0.238$ to $+0.299$), it is significant in $3/3$ seeds for every cell,
and it declines only weakly with $S$ in one direction ($+0.289$ to
$+0.257$, non-monotone) and not at all in the other (flat around
$+0.29$).
...
\begin{tabular}{lccc}
\toprule
$S$ (signers) & real arm & $+$Unreal arm & $\Delta(S)$ \\
\midrule
\multicolumn{4}{l}{\textbf{Single-view panel}: E1 ($K{=}1$) $\to$ E5 ($K{=}7$)} \\
\midrule
...
\midrule
\multicolumn{4}{l}{\textbf{Matched-$K$ panel}: E1 ($K{=}1$) $\to$ E3 ($K{=}3$)} \\
\midrule
\multicolumn{4}{l}{\textit{Test $\to$ Train}} \\
...
\midrule
\multicolumn{4}{l}{\textbf{Multiview panel}: E2 ($K{=}3$) $\to$ E2$+$avatars ($K{=}9$)} \\
\midrule
\multicolumn{4}{l}{\textit{Test $\to$ Train}} \\
...
\bottomrule
\end{tabular}
\caption{Real-signer-count ablation: fixed 180 sentences per condition,
signer count $S{=}1\ldots4$, cross-signer retrieval R@1 (0--1 scale, as
in Table~\ref{tab:ngt_ladder}), mean$\pm$sd over 3 seeds; $\Delta(S)$ =
paired gain of the Unreal arm over the matched real-only arm (unrounded
means). Arms as in Table~\ref{tab:ngt_ladder}: \textbf{single-view} = E1
front camera vs E5 front $+$ 6 avatar views; \textbf{matched-$K$} = the
same E1 vs E3 (front $+$ both avatar front views, the ladder's strongest
arm); \textbf{multiview} = E2 (3 real views) vs E2 $+$ 6 avatar views
--- a viewpoint-matched real baseline. Per-seed exact McNemar (364
paired queries), $p<0.05$: matched-$K$ in $3/3$ seeds for every $S$ and
direction; single-view $3/3$ except $S{=}4$ test$\to$train ($2/3$);
multiview $3/3/2/3$ (test$\to$train) and $3/2/1/2$ (train$\to$test) for
$S{=}1\ldots4$ --- robust test$\to$train, thinning in train$\to$test
exactly where the gain is smallest. Evaluation: the frozen 364-sentence
protocol of Table~\ref{tab:ngt_setups}.
\todome{04.08, from your overflow todo: in-table panel headers shortened
(the multiview one also had a duplicated parenthetical), arm
compositions moved into the caption, McNemar details compressed. No
numbers changed.}}
...
\caption{Unreal appearance gain $\Delta(S)$ against real-signer count, for the
three panels of Table~\ref{tab:ngt_signer_count}: single-view (E1 $\to$ E5,
$K{=}1\to K{=}7$), matched-$K$ (E1 $\to$ E3, $K{=}1\to K{=}3$) and the
viewpoint-matched multiview panel ($K{=}3\to K{=}9$), in each retrieval
direction. Dashed lines are least-squares fits, annotated with slope per
added signer and $R^2$. The full-Unreal and multiview gains shrink as real
signer diversity grows, and the multiview gain is smaller throughout ---
both consistent with the two-regime reading that synthetic appearance
variety substitutes for absent real diversity. The matched-$K$ gain is the
largest at every $S$ and nearly flat in train$\to$test.}
\label{fig:ngt_dropoff}
\end{figure*}

\section{In-domain intervention: ASL Citizen}
\label{sec:results-indomain}
...
\bottomrule
\end{tabular}
\caption{Frozen-feature retrieval on ASL Citizen after backbone
fine-tuning, all on the identical full gallery. In the reference block,
$^{\dagger}$ marks numbers taken from the cited papers (ST-GCN and I3D
trained end-to-end~\cite{aslcitizen}; SignRep on frozen
features~\cite{wongSignRepEnhancingSelfSupervised2025}), all under the
same dictionary-retrieval protocol; those works report retrieval only,
and SignRep does not report R@10. The Logos number is produced by us:
its authors~\cite{ovodovLogosWellTemperedPretrain2025} did not evaluate
on ASL Citizen, so we score their unmodified frozen backbone ourselves.
It is the first of two baseline tiers; the CE-only rows are the second,
standard fine-tuning of the same backbone. Bold marks the numeric best
per column within each block; a blue cell marks the numeric best in the
table for that column (probe values exist only for our rows). These
marks are descriptive, not inferential: each cell is a single training
run, and within-block differences of a few tenths of a point are within
training and evaluation noise; near-ties should be read as equivalent. Row naming: \emph{CE only}
$=$ the unchanged Logos backbone fine-tuned with plain gloss
cross-entropy on the real videos under our recipe; $+$ synth.\ data $=$
CE with the Flux synthetic edits added as training items; $+$ cosine
consistency, $+$ CLS-SupCon, and $+$ SDDA $=$ CE on real $+$ synthetic
data plus the named invariance term; $+$ gloss-SupCon adds a
supervised-contrastive term over same-gloss clips
(\S\ref{sec:method-losses}).
\todome{recommendation 04.08: KEEP DCG, it is the benchmark's signature
metric and the column comparable to the reference systems.}
\todo{unfreeze levels are a separate table, move it. separate question}}
\label{tab:backbone_ft_retrieval}
\end{table*}

\todome{ADDITION START (05.08): credit-framing sentence per your note}
...
is worse than top-6 (85.95), suggesting the earliest, highest-resolution
blocks overfit in-domain signers when adapted. (iii) No appearance or
...
type carries hidden signal. Notably, this null now stands \emph{twice}:
the top-2 rows were fully retrained on the regenerated 512px
augmentations, and the retrained runs moved by only $0.01$--$0.3$
retrieval points (probe values by up to $1.8$) relative to their
first-generation counterparts, reproducing the pattern and calibrating
how little weight the within-block orderings can bear. (iv) Cosine consistency actively damages the
linear probe (probe R@1 drops to 82.27 at top-2 and 82.45 at top-6;
this value also moved by 1.8 points between the two training
generations, the least stable in the table) while its retrieval score
...
rate) fits the same pattern: its retrieval R@1 of 85.02\todome{mismatch
found 05.08: the table's full-unfreeze SDDA R@1 is 85.00, and has been
in every git generation since July; this 85.02 needs the eval log
checked or should follow the table} sits within the
...
training. 
The mechanistic question of whether the consistency loss actually pulls
augmented copies together is now answered directly, not left to this
paragraph: \S\ref{sec:results-ft-probes} (linear decodability) and
\S\ref{sec:results-aug-geometry} (feature-space geometry, training vs.\
held-out pairs) give a converging, load-bearing answer --- yes on training
pairs, not on held-out ones.

\subsection{SignCLIP head on frozen Logos/I3D features}
We next evaluate appearance augmentation as training data for a
contrastive SignCLIP-style head trained on top of frozen backbone features.
...
\toprule
\textbf{Setup} & \textbf{Features} & \textbf{R@1} & \textbf{R@5} & \textbf{R@10} & \textbf{Med.\,R} \\
\midrule
SignCLIP from scratch        & Pose (MediaPipe) & 0.39 & 0.71 & 0.84 & 2 \\
SignCLIP fine-tuned          & Pose (MediaPipe) & 0.46 & 0.77 & 0.84 & 2 \\
SignCLIP fine-tuned + linear & Pose (MediaPipe) & \textbf{0.60} & \textbf{0.84} & \textbf{0.89} & 1 \\
\midrule
From scratch          & RGB (I3D, WLASL)                  & 0.32  & 0.63  & 0.73  & 3 \\
From scratch          & RGB (MViTv2-S, Logos+AUTSL+WLASL) & 0.750 & 0.950 & 0.970 & 1 \\
From scratch $+$ synth.\ data & RGB (MViTv2-S, Logos+AUTSL+WLASL) & 0.756 & 0.955 & 0.970 & 1 \\
Fine-tuned            & RGB (MViTv2-S, Logos+AUTSL+WLASL) & 0.789 & 0.962 & 0.975 & 1 \\
Fine-tuned, synth.\ data only & RGB (MViTv2-S, Logos+AUTSL+WLASL) & 0.760 & 0.957 & 0.971 & 1 \\
Fine-tuned $+$ synth.\ data & RGB (MViTv2-S, Logos+AUTSL+WLASL) & \textbf{0.800} & \textbf{0.964} & \textbf{0.976} & 1 \\
\midrule
\multicolumn{6}{l}{\textit{From-scratch head (real data only) on a \emph{fine-tuned} MViTv2-S backbone}$^{\ddagger}$} \\
Scratch on native (frozen) & RGB (MViTv2-S, native)   & 0.775 & \textbf{0.966} & \textbf{0.977} & 1 \\
Scratch on CE-FT           & RGB (MViTv2-S, CE-FT)    & 0.772 & 0.929 & 0.944 & 1 \\
Scratch on aug-FT          & RGB (MViTv2-S, aug-FT)   & 0.781 & 0.927 & 0.943 & 1 \\
Scratch on SDDA-FT         & RGB (MViTv2-S, SDDA-FT)  & \textbf{0.782} & 0.958 & 0.970 & 1 \\
\midrule
\multicolumn{6}{l}{\textit{Best head recipe (fine-tuned $+$ synth.\ data) on a fine-tuned backbone}$^{\ddagger}$} \\
\rowcolor{rowhl}
FT$+$synth.\ recipe on native   & RGB (MViTv2-S, native)   & \tbest{\textbf{0.818}} & \tbest{\textbf{0.972}} & \tbest{\textbf{0.982}} & 1 \\
FT$+$synth.\ recipe on CE-FT    & RGB (MViTv2-S, CE-FT)    & 0.809 & 0.946 & 0.957 & 1 \\
FT$+$synth.\ recipe on aug-FT   & RGB (MViTv2-S, aug-FT)   & 0.812 & 0.942 & 0.954 & 1 \\
FT$+$synth.\ recipe on SDDA-FT  & RGB (MViTv2-S, SDDA-FT)  & 0.810 & 0.964 & 0.974 & 1 \\
\midrule
\multicolumn{6}{l}{\textit{Pair-SupCon head (fine-tune; synth.\ edits as explicit positives)}} \\
$K{=}3$ pair-SupCon (orig.\ $+$ 2 edits)        & RGB (MViTv2-S, Logos) & 0.567 & 0.887 & 0.934 & 1 \\
\bottomrule
\end{tabular}
\caption{Video-to-text retrieval on ASL Citizen (random-baseline Median R
$=1366$, 2731 unique glosses).
Row naming: \emph{synth.} marks training that uses the Flux synthetic
appearance edits --- \emph{$+$ synth.\ data} adds them to the real training
videos as extra items, \emph{synth.\ data only} fine-tunes on the edits
alone, and the pair-SupCon rows fine-tune with a SupCon objective treating
each video and its synthetic edits as explicit positives; unmarked rows
train on real videos only. The three pose-input rows are the published
results of the original SignCLIP model~\cite{jiangSignCLIPConnectingText2024},
not our runs.\todome{provenance sentence added 04.08 per your note ---
verify the three pose numbers (0.39/0.46/0.60) against the SignCLIP
paper's tables before sending}
$^{\ddagger}$These heads train the named recipe on \emph{fine-tuned}
(rather than frozen) MViTv2-S features (backbone tags: CE-FT $=$
cross-entropy on real videos; aug-FT $=$ CE $+$ synth.\ data; SDDA-FT $=$
CE $+$ synth.\ data $+$ SDDA invariance term,
\S\ref{sec:method-losses}); the CE/aug/SDDA-FT rows use
\texttt{logos\_native} preprocessing, whereas the frozen rows above use
\texttt{direct224} --- the ``Scratch on native (frozen)'' row is the matched
control that isolates the preprocessing difference from the fine-tuning effect.
\todome{K=5 removal executed 04.08: the K=5 pair-SupCon row (0.414,
checkpoint\_last) and its dangling $^{\S}$ mark are gone from this
table, and the K=3-to-K=5 retrieval citation in Future Work was
rephrased. BOUNDARY QUESTION for you: the K=5 \emph{model} still appears
in non-retrieval places --- the head signer-probes table (26.34$\times$
/ 21.2\% row, and the caption's 22.0--26.3$\times$ range) and two
appendix iconicity analyses of its embedding space. Same checkpoint
provenance. Remove the model everywhere, or keep the embedding-space
rows? Say the word and I sweep the rest.}
}
...
\todome{DRAFT PROPOSAL START (03.08): this paragraph header had no body;
the text below states only what the two tables show, no new claims.}
Table~\ref{tab:asl_citizen_t2v} repeats the comparison in the
text-to-video direction, where precision@$k$ replaces recall@$k$ because
the gallery holds several correct videos per gloss query. The ordering
matches the video-to-text table: fine-tuning alone lifts P@1 from
$0.871$ to $0.895$, synthetic data alone adds little ($0.883$ from
scratch, $0.884$ as the only fine-tuning data), and the combination is
the strongest configuration on every metric (P@1 $0.919$, Mean~R
$1.128$). Carried onto the fine-tuned backbones, the same recipe reaches
P@1 $0.925$--$0.935$, mirroring its video-to-text behaviour.
...
\begin{table*}[t]
  ...
  \begin{tabular}{llcccc}
  \toprule
  \textbf{Setup} & \textbf{Features} & \textbf{P@1} & \textbf{P@5} & \textbf{P@10} & \textbf{Mean R} \\
  \midrule
  From scratch          & RGB (MViTv2-S, Logos+AUTSL+WLASL) & 0.871 & 0.854 & 0.786 & 1.228 \\
  From scratch $+$ synth.\ data    & RGB (MViTv2-S, Logos+AUTSL+WLASL) & 0.883 & 0.861 & 0.790 & 1.213 \\
  Fine-tuned           & RGB (MViTv2-S, Logos+AUTSL+WLASL) & 0.895 & 0.876 & 0.808 & 1.166 \\
  Fine-tuned, synth.\ data only & RGB (MViTv2-S, Logos+AUTSL+WLASL) & 0.884 & 0.863 & 0.796 & 1.216 \\
  Fine-tuned $+$ synth.\ data      & RGB (MViTv2-S, Logos+AUTSL+WLASL) & \textbf{0.919} & \textbf{0.896} & \textbf{0.829} & \textbf{1.128} \\
  \midrule
  \multicolumn{6}{l}{\textit{Best head recipe (fine-tuned $+$ synth.\ data) on a fine-tuned backbone} }\\

  FT$+$synth.\ recipe on native   & RGB (MViTv2-S, native)   & 0.925 & 0.902 & \tbest{\textbf{0.850}} & 1.129 \\
  \rowcolor{rowhl}

  FT$+$synth.\ recipe on CE-FT    & RGB (MViTv2-S, CE-FT)    & \tbest{\textbf{0.935}} & 0.909 & 0.844 & 1.106 \\
  FT$+$synth.\ recipe on aug-FT   & RGB (MViTv2-S, aug-FT)   & 0.930 & \tbest{\textbf{0.910}} & 0.848 & \tbest{\textbf{1.099}} \\
  FT$+$synth.\ recipe on SDDA-FT  & RGB (MViTv2-S, SDDA-FT)  & 0.928 & 0.906 & 0.849 & 1.112 \\
  \bottomrule
  \end{tabular}
  }
  \caption{Text-to-video retrieval on ASL Citizen, same configurations as
  Table~\ref{tab:asl_citizen_retrieval}. Reported as precision@k rather than
  recall@k: the test gallery contains multiple correct videos per gloss query,
  so recall@1 is bounded by gallery multiplicity regardless of model quality,
  while precision@k directly measures whether the top-ranked video(s) are
  correct. The original SignCLIP work does not report this direction for ASL-Citizen at
  full scale.}
...
Figure~\ref{fig:tsne_grid} gives a qualitative view of the gloss
organisation of nine of these spaces on a shared clip sample.
...
\caption{t-SNE projections (shared hyper-parameters: perplexity 30, PCA
...
\bottomrule
\end{tabular}
\caption{Signer-identity probes on the re-extracted top-2-family spaces.
...
\bottomrule
\end{tabular}
\caption{The same two probes on the SignCLIP-head spaces. Head features
...
decodable signer information and improving in-domain accuracy are
...
The interventions above add little in-domain (\S\ref{sec:results-indomain}),
...
(source excluded) show the same sign? --- the edit preserved meaning; the
...
nearest prototype differs between source and augment --- the edit visibly
...
artifact. What we deliberately do \emph{not} remove is the diffusion
...
question for the in-domain null is \emph{where} that displacement lands. If
...
low-signer NGT result (\S\ref{sec:results-crossdomain}). If instead the
...
\bottomrule
\end{tabular}
\caption{Augmentation geometry in the \textbf{frozen} Logos space
...
is the mechanistic reading behind the in-domain null: the entire
...
\toprule
& \multicolumn{3}{c}{\textbf{Training pairs}} &
\multicolumn{3}{c}{\textbf{Held-out pairs}} & \\
\cmidrule(lr){2-4}\cmidrule(lr){5-7}
\textbf{Space} & n.disp & gloss@1 & AUC & n.disp & gloss@1 & AUC &
\textbf{AUC$_{\text{de-sh}}$} \\
\midrule
frozen      & 1.04 & 0.49 & 0.997 & 1.05 & 0.59 & 0.976 & 0.50 \\
CE          & 1.13 & 0.56 & 0.992 & 1.00 & 0.60 & 0.972 & 0.50 \\
aug-as-data & 1.04 & 0.91 & 0.991 & 1.17 & 0.62 & 0.949 & 0.48 \\
consistency & 0.79 & 0.89 & 0.986 & 1.11 & 0.57 & 0.942 & 0.49 \\
SDDA        & 0.76 & 0.67 & 0.996 & 0.80 & 0.66 & 0.961 & 0.50 \\
\bottomrule
\end{tabular}
\caption{The same geometry across feature spaces, shown for the
...
contract n.disp and preserve gloss@1 in \emph{both} column groups;
gains confined to the training columns (aug-as-data, consistency) are
...
\todo{Add the plots from the presentation}
\todo{Discuss how all interventions reduce iconicity}
\todo{describe high iconicity signs based on ASL-LEX, include an example of high-iconicity family used in the presentation}
The organising
construct of the Logos dataset is the \emph{visually similar sign}
(VSSign): signs with different meanings but the same manual component,
subsuming both polysemy and minimal pairs, and the Logos authors show
