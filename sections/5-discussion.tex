...
\todome{REWRITTEN (12.08) to your clarification: the point is replacing
the \emph{motion source}, not the avatar assets. The previous version
proposed dropping CharacterCreator for SMPL-X bodies, which was not the
intended claim.}
\textbf{Recovering motion from video rather than from motion capture.}
The binding constraint on the motion-locked route is not the avatar but
the capture: it needs recorded motion capture, which is why it could not
be run on ASL Citizen (\S\ref{sec:method-diffusion}) and why the
generative edits exist as an approximation at all. Estimating parametric
SMPL-X~\cite{pavlakosExpressiveBodyCapture2019a} pose and shape directly
from RGB video would remove that constraint: the recovered parameters
could drive the same rendering pipeline on any corpus, with the avatars
themselves unchanged. SignAvatars~\cite{yuSignAvatarsLargeScale3D2025}
and Neural Sign Actors~\cite{baltatzisNeuralSignActors2024a} indicate the
direction, supplying sign-specific SMPL-X data and generation in that
space, but video-to-parameter
estimation~\cite{kanazawaEndtoEndRecoveryHuman2018} would have to improve
first: sign meaning depends on fine handshape and facial expression,
precisely what body-level estimators reconstruct least reliably.
