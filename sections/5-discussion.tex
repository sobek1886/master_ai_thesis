...
\todome{ADDITION START (Claude 12.08): future-work counterpart of the
co-training conjecture in the discussion.}
\textbf{Does co-training preserve what fine-tuning flattens?} The
conjecture raised in \S\ref{sec:discussion-regimes}, that co-training
transfers better than fine-tuning partly because it preserves the
pretrained form organisation, is directly testable without any new data.
Starting from the same pretrained checkpoint, train one target model by
fine-tuning and one by co-training with the source objective retained,
then measure both target accuracy and the form-dose response
(\S\ref{sec:results-iconicity-main}) at matched accuracy. The conjecture
predicts that the two reach comparable accuracy while only the fine-tuned
space loses its form organisation. Running it across pretrain and target
pairs of varying vocabulary overlap would also separate the two
explanations we cannot currently distinguish, whether the small in-domain
gain reflects a target vocabulary already covered by pretraining or a
ceiling on what fine-tuning can add at all.
\todome{ADDITION END}
