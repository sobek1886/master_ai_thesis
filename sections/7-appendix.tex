...
\todome{TRIMMED 12.08 (Claude): the form-dose measure, its dose-response
and the per-space comparison all moved to the main text
(\S\ref{sec:results-iconicity-main},
Figs.~\ref{fig:form_dose_ft} and~\ref{fig:form_dose_rho}); the old
three-panel figure duplicated them and is dropped.}
Before testing iconicity itself we measure how strongly each space is
organised by phonological \emph{form}, using the form-dose measure
defined in \S\ref{sec:results-iconicity-main}: for every pair of gloss
centroids, count how many of the five ASL-LEX~2.0 form
features~\cite{sehyrASLLEX2021} the two glosses share (movement, major
and minor location, handshape, sign type; $0$--$5$) and bucket the
pairwise cosine similarities by that count (\texttt{form\_dose.py}).
Figures~\ref{fig:form_dose_ft} and~\ref{fig:form_dose_rho} report the
result for the five headline spaces and for every space we trained
respectively. The iconicity test below is run on the same spaces.
...
\todome{RESTORED 12.08 (Claude): this results passage and its figure existed
in the pre-move draft but were dropped when the thesis moved to this repo,
leaving the section with a method and no findings (and
\texttt{thesis\_iconicity\_contrast.png} unreferenced in media/figures).
Ported verbatim apart from the added short caption.}
...
\caption[Matched high-low iconicity contrasts across spaces]{Matched
high$-$low iconicity contrasts (248 pairs, deaf-signer
...
\todome{MOVED 12.08 (Claude): the t-SNE grid was demoted from Results to
here; Results now points at it. Caption unchanged.}
