...
and CSL-Daily~\cite{CSL-daily}. \citet{jangSigningOutsideStudio2022}
note the limitations of studio-based benchmarks, in particular their lack of
variance in setting and background; ~\citet{CE-CNSL} similarly identify the
studio-setting nature of existing benchmarks as a bottleneck and release
CE-CNSL to address it. More recently, \citet{alyamiIsharahLargeScaleMultiScene2026b} address the
lack of diversity and the challenge of signer-independent
evaluation, and their Isharah dataset is used in the
...
aggregates recordings under varied real-world conditions. \todo{check if this reference adds anything} ~\citet{atwellStudyingMitigatingBiases2024}
...
for future bias work.
\todo{this section reads kinda weirdly. It's original premise was showing that the RGB-based models performed better on those datasets with less variety, but then more diverse datasets came and with them a shift towards pose representation}
...
\citet{byvshevAre3DConvolutional2022a} show that the appearance bias observed in 3D convolutional video models
is not an inherent architectural limitation but a consequence of the
statistical structure of training data:
when motion and appearance are artificially decoupled, 3D CNNs learn
temporally sensitive representations. They are not alone in
locating the bias in data rather than architecture: ~\citet{kowalQuantifyingLearningStatic2025} find most spatiotemporal models dominated by static information and
reduce that dominance with a training-time
intervention; \citet{liMitigatingEvaluatingStatic2023a} show
that a static-bias-mitigating augmentation works on a video Swin
transformer~; and a family of
...
\todo{introducing signrep here really breaks the flow} SignRep~\cite{wongSignRepEnhancingSelfSupervised2025} argues that the
