...
intervention on NGT775 where synthetic appearance augmentation produces a
...
\section{Cross-domain intervention: NGT775}
...
matched $K$, matched content, and matched viewpoint; a further
full-strength arm adds all six avatar views ($K{=}7$). All arms share an
...
\bottomrule
\end{tabular}
\caption{Single-view NGT775 ladder: cross-signer retrieval (0--1 scale) with viewpoint
...
\caption{Single-view NGT775 ladder: cross-signer R@1 against the number of
...
add. The NGT775 corpus permits a direct test: its training pool comprises
...
low-signer NGT775 result (\S\ref{sec:results-crossdomain}). If instead the
