...
the generative appearance edits: \emph{$+$ synth.\ data} adds them to the real
...
changes nothing but the training split, adding edited clips to
the real ones.
...
Logos features. They resume the same from-scratch head as the
...
and $0.414$ with four. That gap is not a verdict on the edits. This stage
drops the text tower and every form of gloss supervision
(\S\ref{sec:method-objectives}) and is still scored on video-to-text
retrieval, so a large loss is expected before a single edit is added, and
no run separates the two. What the two rows do compare is each other:
only the number of edits per anchor differs, and more of them is worse
...
computation above. Against the train-split frozen ratio of $22.7\times$
(Table~\ref{tab:raw_feature_l2_probe}), the heads leave the distance
geometry near the frozen level ($22.0$--$23.4\times$), with only the
$K{=}5$ pair-SupCon head clearly above it ($26.3\times$), while linear
decodability falls from $66.9\%$ to $20$--$25\%$.}
...
largely survives the appearance edits (gloss retention $0.70$--$0.75$ on
the unbiased held-out pairs for glasses, shirt and skin tone) but takes a
real hit from the identity change ($0.59$ for signer swap); on the
training pairs, which by construction target the hardest classes,
retention is lower throughout ($0.49$--$0.63$). That the strongest edit
type is also the one that costs the most sign content matches the
hallucination risk flagged for it in advance
(\S\ref{subsection:GAE}). Yet the same augments are
