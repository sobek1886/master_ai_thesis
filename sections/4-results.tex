...
Figure~\ref{fig:form_dose_ft} compares the frozen backbone with the two
fine-tuning recipes. The frozen space responds strongly: mean centroid
similarity climbs from $0.01$ at zero shared features to $0.18$ at five
(Spearman $\rho=.06$ over all pairs, $n{=}2{,}178$ at dose five). Both
fine-tuning recipes flatten the response, CE almost completely ($0.09$
at dose five, $\rho=.00$), SDDA partially ($0.13$, $\rho=.03$). The
form and iconicity organisation is therefore a property of the
pretrained space, and in-domain training weakens it rather than
building on it; none of our interventions raises it above the frozen
level. The full thirteen-space grid, and the matched-pair iconicity
battery behind that last claim (the high-versus-low-iconicity contrast
is present in the frozen space, washed out by text alignment, and only
partially restored by pair training), are in
Appendix~\ref{sec:results-iconicity}.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{violin_raw_ce_sdda.png}
\caption[Form-dose response: frozen vs fine-tuned]{Centroid cosine
similarity against the number of shared ASL-LEX form features (pair
counts below each level), for the frozen backbone and the two
fine-tuning recipes. Fine-tuning flattens the dose-response: at five
shared features the mean drops from $0.18$ (frozen) to $0.13$ (SDDA)
and $0.09$ (CE).}
