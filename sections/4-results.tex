...
Those two blocks were generated with a different frame preprocessing
from the rows above, so each carries its own frozen-backbone row as the
matched control and is read within the block.
...
$0.789$). All of these differences are one to three points.

The last two blocks search for the best combination, putting a head on
backbones we had already fine-tuned. Their answer is that the two do not
stack. A from-scratch head does gain from a fine-tuned backbone ($0.744$
...
The pair-SupCon rows are the exception and the largest effect in the
table: the same edits, entering as explicit positives instead of as data,
cost $0.233$ R@1 at $K{=}3$ and $0.386$ at $K{=}5$ against the $0.800$
they support as data (\S\ref{sec:results-iconicity-main} returns to
why).
...
\midrule
From scratch          & RGB (I3D)                  & 0.32  & 0.63  & 0.73  & 3 \\
From scratch          & RGB (Logos) & 0.750 & 0.950 & 0.970 & 1 \\
From scratch $+$ synth.\ data & RGB (Logos) & 0.756 & 0.955 & 0.970 & 1 \\
Fine-tuned            & RGB (Logos) & 0.789 & 0.962 & 0.975 & 1 \\
Fine-tuned, synth.\ data only & RGB (Logos) & 0.760 & 0.957 & 0.971 & 1 \\
Fine-tuned $+$ synth.\ data & RGB (Logos) & \textbf{0.800} & \textbf{0.964} & \textbf{0.976} & 1 \\
\midrule
\multicolumn{6}{l}{\textit{From-scratch head, varying the backbone underneath}$^{\ddagger}$} \\
From scratch              & RGB (Logos, frozen)   & 0.744 & \textbf{0.966} & \textbf{0.977} & 1 \\
From scratch              & RGB (Logos, CE-FT)    & 0.772 & 0.929 & 0.944 & 1 \\
From scratch              & RGB (Logos, aug-FT)   & 0.781 & 0.927 & 0.943 & 1 \\
From scratch              & RGB (Logos, SDDA-FT)  & \textbf{0.782} & 0.958 & 0.970 & 1 \\
\midrule
\multicolumn{6}{l}{\textit{Best head recipe, varying the backbone underneath}$^{\ddagger}$} \\
\rowcolor{rowhl}
Fine-tuned $+$ synth.\ data & RGB (Logos, frozen)   & \tbest{\textbf{0.818}} & \tbest{\textbf{0.972}} & \tbest{\textbf{0.982}} & 1 \\
Fine-tuned $+$ synth.\ data & RGB (Logos, CE-FT)    & 0.809 & 0.946 & 0.957 & 1 \\
Fine-tuned $+$ synth.\ data & RGB (Logos, aug-FT)   & 0.812 & 0.942 & 0.954 & 1 \\
Fine-tuned $+$ synth.\ data & RGB (Logos, SDDA-FT)  & 0.810 & 0.964 & 0.974 & 1 \\
\midrule
\multicolumn{6}{l}{\textit{Pair-SupCon head (fine-tune; synth.\ edits as explicit positives)}} \\
$K{=}3$ pair-SupCon (orig.\ $+$ 2 edits)        & RGB (Logos) & 0.567 & 0.887 & 0.934 & 1 \\
$K{=}5$ pair-SupCon (orig.\ $+$ 4 edits)        & RGB (Logos) & 0.414 & 0.767 & 0.858 & 2 \\
\bottomrule
\end{tabular}
...
glosses). The \textbf{Features} column names the backbone the head is
trained on and, where relevant, how that backbone was itself fine-tuned;
row naming is explained in the text. $^{\ddagger}$These two blocks were
generated with a different frame preprocessing from the rows above, so
they should be read within the block, against their own frozen-backbone
row, rather than against the earlier numbers. Bold marks the best value in
a column within its block, a blue cell the best in the column overall, and
the shaded row the strongest single configuration.}
...
\begin{table*}[t]
  ...
  \midrule
  From scratch          & RGB (Logos) & 0.871 & 0.854 & 0.786 & 1.228 \\
  From scratch $+$ synth.\ data    & RGB (Logos) & 0.883 & 0.861 & 0.790 & 1.213 \\
  Fine-tuned           & RGB (Logos) & 0.895 & 0.876 & 0.808 & 1.166 \\
  Fine-tuned, synth.\ data only & RGB (Logos) & 0.884 & 0.863 & 0.796 & 1.216 \\
  Fine-tuned $+$ synth.\ data      & RGB (Logos) & \textbf{0.919} & \textbf{0.896} & \textbf{0.829} & \textbf{1.128} \\
  \midrule
  \multicolumn{6}{l}{\textit{Best head recipe (fine-tuned $+$ synth.\ data) on a fine-tuned backbone} }\\

  Fine-tuned $+$ synth.\ data & RGB (Logos, frozen)   & 0.925 & 0.902 & \tbest{\textbf{0.850}} & 1.129 \\
  Fine-tuned $+$ synth.\ data & RGB (Logos, CE-FT)    & \tbest{\textbf{0.935}} & 0.909 & 0.844 & 1.106 \\
  Fine-tuned $+$ synth.\ data & RGB (Logos, aug-FT)   & 0.930 & \tbest{\textbf{0.910}} & 0.848 & \tbest{\textbf{1.099}} \\
  Fine-tuned $+$ synth.\ data & RGB (Logos, SDDA-FT)  & 0.928 & 0.906 & 0.849 & 1.112 \\
  \bottomrule
  \end{tabular}
  }
