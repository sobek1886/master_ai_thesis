...
$+0.134$ train$\to$test), and the appearance-over-real-viewpoints panel repeats
the pattern from a stronger real baseline, with smaller gains throughout.
The appearance-only arm does not: it is the strongest configuration at every
...
means). Each panel pairs a real-only
baseline with the same arm plus motion-locked avatars, so the panels differ
in what the avatars are free to add: identity alone at a fixed camera
(\emph{appearance only}), identity and viewpoint together
(\emph{appearance $+$ viewpoint}), or identity on top of a baseline that
already has real viewpoint diversity (\emph{appearance over real
viewpoints}); arm codes refer to
Table~\ref{tab:ngt_synthetic_views}, except E2$+$av., the E2 arm carrying
the same six avatar views ($K{=}9$), which has no counterpart
there. Per-seed exact McNemar (364 paired
queries), $p<0.05$: appearance-only in $3/3$ seeds for every $S$ and
direction; appearance $+$ viewpoint $3/3$ except $S{=}4$
test$\to$train ($2/3$); appearance over real viewpoints $3/3/2/3$
(test$\to$train) and $3/2/1/2$ (train$\to$test) for $S{=}1\ldots4$:
robust test$\to$train, thinning in train$\to$test exactly where the gain
is smallest. Evaluation: the frozen 364-sentence
...
diversity. The appearance-only gain is the largest at every $S$ and
nearly flat in train$\to$test.}
