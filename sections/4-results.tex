...
The synthetic-view arms vary which views form the positive set: their
source (real camera, Unreal avatar, generative edit) and their number
$K$. The matched-viewpoint comparison that isolates appearance alone is
E1 vs.\ E3 vs.\ E4, where every view sits at the front camera. The
baseline trains on the front camera alone ($K{=}1$; with no
...
\bottomrule
\end{tabular}
\caption{Synthetic-view arms on NGT744: cross-signer retrieval (0--1 scale),
mean$\pm$sd over 3 seeds. Arm composition ($K$ =
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{ngt_synthetic_views_r1_vs_k.png}
...
$+0.257$ / $+0.363$).
\todome{NUMBERS BUG (03.08, verified against tab:ngt\_synthetic\_views): the
...
the actual E1$\to$E3 and E1$\to$E4 McNemar results from the synthetic-view eval
...
matched-$K$ arm (the synthetic-view E3 setup, $K{=}3$: front camera plus both
...
synthetic-view arms (Table~\ref{tab:ngt_setups}),
...
in Table~\ref{tab:ngt_synthetic_views}), mean$\pm$sd over 3 seeds; $\Delta(S)$ =
paired gain of the Unreal arm over the matched real-only arm (unrounded
means). Arms as in Table~\ref{tab:ngt_synthetic_views}: \textbf{single-view} = E1
front camera vs E5 front $+$ 6 avatar views; \textbf{matched-$K$} = the
same E1 vs E3 (front $+$ both avatar front views, the synthetic-view arms' strongest
