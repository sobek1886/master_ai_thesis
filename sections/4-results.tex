...
ASL Citizen contains such groups too, and they sit at the top of the
iconicity scale. Figure~\ref{fig:iconicity_family} shows one:
GUN1, LIGHTER and REMOTECONTROL agree on all five ASL-LEX form features
(neutral location, \texttt{a} handshape, no path movement, one-handed)
because each depicts handling the object it names, and all three carry
high deaf-signer iconicity ratings ($5.3$--$5.6$ of $7$). Iconic
vocabulary is thus exactly where distinct meanings converge on the same
form, so how a space treats form-similar pairs decides what it can say
about iconic signs.
...
We quantify this treatment with the form-dose measure of
Appendix~\ref{sec:results-iconicity}: bucket every pair of gloss
centroids by how many of the five form features the glosses share and
trace cosine similarity against that count.
...
tell apart. The matched-pair iconicity battery in
Appendix~\ref{sec:results-iconicity} shows the consequence for iconicity
itself, present in the frozen space, washed out by text alignment, and
partially restored by pair training.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{form_dose_main.png}
\caption[Form-dose response across training recipes]{Centroid cosine
similarity against the number of shared ASL-LEX form features (pair
counts below each level; $\rho$ is the Spearman correlation over all
centroid pairs). The frozen backbone sits between two opposite
...
slope rather than the absolute level.}
