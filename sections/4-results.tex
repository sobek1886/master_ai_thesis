...
training videos as extra items, and the pair-SupCon rows use them as
explicit positives (\S\ref{sec:method-objectives}); unmarked rows train on
real videos only.
...
describes what the recipes differ in, read off the run configs. One
thing surfaced while checking: \emph{from scratch} initialises both towers
from pretrained BERT rather than randomly. The \emph{synth.\ data only} row
(R@1 $0.760$, P@1 $0.884$) was dropped from both tables on 12.08: it trains
on 450 of the 2{,}731 glosses, so it is not a like-for-like comparison and
only invited misreading. A separate check on whether the recipe search used
the preprocessing the Logos authors use is still open.}
Every cell in both tables is a single training run, without seeds or
confidence intervals, and the same configuration appears twice at $0.800$
and $0.818$ under the two frame preprocessings, so differences below
roughly two points are not interpreted below.
The four rows differ in only two respects: how many training stages the
...
the $40{,}154$ real ones. Those clips are edits of $450$ of the $2{,}731$
glosses, so the intervention reaches a sixth of the vocabulary.

Read this way, the second stage is worth $3.9$ points ($0.750$ to
$0.789$) and the edits roughly one more on top of it ($0.789$ to
$0.800$). Both are small beside the choice of
...
not stack. At R@1 a from-scratch head gains from a fine-tuned backbone
($0.744$ on the matched frozen control against $0.772$--$0.782$), but the
ordering reverses at R@5 and R@10, where the frozen backbone leads
($0.966/0.977$ against $0.927$--$0.958$ and $0.943$--$0.970$), so the gain
is a top-1 effect rather than a better-ordered gallery. The best head
recipe shows no gain at all: it scores highest on the frozen backbone
($0.818$) and slightly lower on every fine-tuned one ($0.809$--$0.812$),
all within the two-point range above. Whatever backbone fine-tuning has
to contribute at this point, the head has already contributed.

The pair-SupCon rows are the largest effect among the head recipes on
Logos features, and they are the cleanest comparison in the table: they resume the same from-scratch head as the
...
%   {From scratch | Fine-tuned} x {(real only) | + synth. data},
%   "synth. data" (not "aug") everywhere.
...
which is the head's second training stage, not backbone fine-tuning. It also
read the now-removed synth-only row as evidence that synthetic data adds
little.}
...
($0.883$). Carried onto the fine-tuned backbones, the same recipe reaches
P@1 $0.928$--$0.935$, against $0.925$ on the frozen backbone. Which
backbone leads is the one place the two directions disagree: the frozen
backbone is strongest video-to-text ($0.818$ against $0.809$ on CE-FT)
while CE-FT is strongest text-to-video ($0.935$ against $0.925$). Both
margins are about one point, so we note the disagreement without reading
a preference into it.
\todome{ADDED 12.08, replacing a proposed error-reduction paragraph. An
examiner pass rejected that version: the two error rates are computed
over different query sets ($32{,}941$ videos against $2{,}731$ glosses),
so the ratio between their error reductions is not a quantity, and the
mechanism it offered was circular. The likeliest explanation, that in
text-to-video the queries \emph{are} glosses and only $450$ of $2{,}731$
were edited, is stated here as a conjecture; splitting P@1 by edited and
untouched glosses would test it directly and is an afternoon's work.}
The edits move P@1 more than they move R@1 ($0.024$ against $0.011$). We
do not read a ratio between the two: the two error rates are computed over
different query sets, and both effects are of the same order as the
$0.018$ that a change of preprocessing alone moves the same configuration
in Table~\ref{tab:asl_citizen_retrieval}. One structural difference is
worth recording for future work: the edits cover $450$ of the $2{,}731$
glosses, and in text-to-video the queries are glosses, so most of that
query set was never touched by the intervention.
...
\begin{table*}[t]
  ...
  retrieval on ASL Citizen for a subset of the configurations of
  Table~\ref{tab:asl_citizen_retrieval}; the pose, I3D and pair-SupCon
  rows are not repeated here. Bold marks the best value per column within
  a block and a blue cell the best in the column. As in
  Table~\ref{tab:asl_citizen_retrieval}, the lower block was generated
  with a different frame preprocessing from the rows above and is read
  within the block.}
