...
\todome{ADDITION START (Claude 12.08): setup paragraph and the reading of
the video-to-text table. The setup text is moved out of the table caption,
which previously carried the row naming, the pose provenance and the
$\ddagger$ footnote; the reading is new, the subsection had none.}
...
is why they are reported to two decimals where ours carry three. The lower
two blocks train the same head recipes on \emph{fine-tuned} rather than
frozen features, tagged by how that backbone was fine-tuned: CE-FT is
cross-entropy on the real videos, aug-FT adds the synthetic edits as data,
and SDDA-FT adds the SDDA invariance term (\S\ref{sec:method-losses}).
Those two blocks use \texttt{logos\_native} preprocessing where the rows
above use \texttt{direct224}, so the ``Scratch on native (frozen)'' row is
included as the matched control that separates the preprocessing change
from the effect of fine-tuning the backbone.

The head is worth adding, and the edits help it a little. On frozen Logos
features a head trained from scratch already reaches $0.750$ R@1, far
above the same head on I3D features ($0.32$) or the published pose-input
SignCLIP models ($0.39$--$0.60$). Fine-tuning the head lifts this to
$0.789$, and adding the synthetic edits as extra training pairs to
$0.800$, the best configuration on frozen features; training on the edits
alone is worse than training on the real videos ($0.760$ against
$0.789$). Carrying the same recipe onto fine-tuned backbones adds a
little more, peaking at $0.818$. Every one of these differences is
between one and three points. The pair-SupCon rows are the exception and
the largest effect in the table: the same edits, entering as explicit
positives instead of as data, cost $0.233$ R@1 at $K{=}3$ and $0.386$ at
$K{=}5$ against the $0.800$ they support as data
(\S\ref{sec:results-iconicity-main} returns to why).
\todome{ADDITION END}
...
$K{=}5$ pair-SupCon (orig.\ $+$ 4 edits)        & RGB (MViTv2-S, Logos) & 0.414 & 0.767 & 0.858 & 2 \\
\bottomrule
\end{tabular}
\caption[Video-to-text retrieval on ASL Citizen]{Video-to-text
retrieval on ASL Citizen (random-baseline Median R $=1366$, 2731 unique
glosses); row naming and the backbone tags marked $^{\ddagger}$ are
explained in the text. Bold marks the best value in a column within its
block, a blue cell the best in the whole column, and the shaded row the
strongest configuration overall.\todome{open since 04.08: verify the
three published pose numbers (0.39/0.46/0.60) against the SignCLIP
paper's tables.}}
...
P@1 $0.925$--$0.935$, mirroring its video-to-text behaviour. Which of
those backbones leads does depend on the direction, however: the recipe on
native features is strongest video-to-text ($0.818$ against $0.809$ on
CE-FT) while CE-FT is strongest text-to-video ($0.935$ against $0.925$),
so the shaded row differs between the two tables.
