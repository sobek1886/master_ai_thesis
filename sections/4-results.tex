...
\bottomrule
\end{tabular}
\caption{Synthetic-view arms on NGT744: cross-signer retrieval (0--1
...
blocks overfit the training signers when adapted. (iii) No appearance or
...
\todome{ADDITION START (Claude 12.08): iconicity subsection body. Numbers
recomputed from the buckets\_*.npz artifacts on the cluster (dose-5 means
raw 0.179 / SDDA 0.126 / CE 0.089; Spearman .064/.034/.002; 2,178 pairs);
they match the appendix and the presentation figure.}
To test what that bias does to an ASL space we need a description of sign
form that is independent of it. ASL-LEX~2.0 provides
one~\cite{sehyrASLLEX2021,caselliASLLEX2017}: a linguistic database that
codes each of $2{,}723$ ASL signs on five phonological features
(movement, major and minor location, handshape, sign type) and rates its
iconicity, how much its form resembles its meaning, on a $1$--$7$ scale
from deaf signers. The difference from the VSSign premise matters twice
over. VSSign membership is a binary judgement about Russian signs, made
by the Logos annotators and used as \emph{training labels}; ASL-LEX is an
external coding of American signs that we use only as a
\emph{measurement}, and because it codes five features separately it
turns ``looks alike'' into a graded quantity, from zero to five shared
features, rather than a yes/no. Measuring an ASL space this way is
therefore not a restatement of the backbone's supervision: whatever form
organisation we find has transferred across languages and across
annotation schemes.
...
We quantify this treatment with a \emph{form-dose} measure: bucket every
pair of gloss centroids by how many of the five features the two glosses
share, and trace cosine similarity against that count.
Figure~\ref{fig:form_dose_ft} places the frozen backbone between the two
kinds of training we apply to it, and they move in opposite directions.
The frozen space responds strongly on its own: mean centroid similarity
climbs from $0.01$ at zero shared features to $0.18$ at five (Spearman
$\rho=.06$ over all pairs, $n{=}2{,}178$ at dose five). Fine-tuning the
backbone flattens that response, and plain cross-entropy flattens it
most ($0.09$ at dose five, $\rho=.00$), with SDDA retaining about half
the frozen slope ($0.13$, $\rho=.03$); the same holds for the two
recipes not shown here, aug-as-data and cosine consistency
($\rho=.01$ each, Appendix~\ref{sec:results-iconicity}). Training a head
with the counterfactuals as explicit positives does the reverse and
overshoots the frozen level, more so with more positives per item
($0.25$ at $K{=}3$, $\rho=.09$; $0.39$ at $K{=}5$, $\rho=.16$).

Figure~\ref{fig:form_dose_rho} reduces every space we trained to the same
single number and shows the split is systematic, not particular to the
three recipes above. All four backbone fine-tunes fall below the frozen
level, and the plain one falls furthest. Text-aligned SignCLIP heads on
the frozen backbone stay close to it, and heads built on already
fine-tuned backbones inherit the flattening rather than recovering from
it. Only the pair-SupCon heads exceed the frozen level, and only they
were trained with the counterfactuals as explicit positives.

This is the same trade-off the retrieval tables record, seen from the
other side, and the pair-SupCon result is the more surprising half.
That objective carries no gloss supervision whatsoever: it replaces the
video--text loss (\S\ref{sec:method-objectives}) with a contrast whose
positives are only the $K$ views of a single source video, its original
and its appearance edits, and whose negatives are every other item in the
batch, \emph{including other recordings of the same sign}. Two GUN1
videos are pushed apart exactly as GUN1 and LIGHTER are. The rise in form
organisation is therefore emergent rather than trained: collapsing
appearance variation within each item removes the axis the space was
mostly using, and visual form is what remains to organise it, so
form-similar glosses drift together as a side effect of an objective that
never grouped them. It is bought at $0.567$ against $0.800$ V2T R@1 for
the same edits used as ordinary training data.
Cross-entropy fine-tuning applies the opposite pressure, pulling
glosses apart whatever they look like, and buys $2.4$ retrieval points
while taking the form response to zero. Neither extreme is the goal:
form organisation is what makes an embedding space look semantic on
iconic vocabulary, and it is also what makes form-similar signs hard to
tell apart.

That first half deserves stating plainly, because it bounds what any
embedding-space evaluation in this literature can claim. For a highly
iconic sign, looking alike and meaning alike are the same thing: whatever
groups HAMMER with hammering-like motion needs no concept of hammering.
A purely visual space will therefore appear semantic when probed on
iconic vocabulary, and only on arbitrary signs, where form and meaning
come apart, does the distinction become testable. Reports of
``semantically meaningful'' neighbourhoods in sign-video embedding spaces
are hard to interpret unless iconicity is controlled, and the spread is
wide enough to matter: within ASL Citizen's vocabulary, ASL-LEX rates
EYEGLASSES at $6.77$ and SCISSORS at $6.03$, against NEPHEW at $1.14$ and
COMPUTER at $1.84$. A matched-pair test on iconicity itself
(Appendix~\ref{sec:results-iconicity}, 248 pairs balanced for lexical
class and frequency) points the same way as the form-dose result, but its
effects are small and only the location-overlap contrasts survive
multiple comparisons, so we treat it as corroborating rather than
load-bearing.
